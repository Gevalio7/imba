const { pool } = require('../config/db');
  ...
  static async create(dynamicfieldsscreen) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`INSERT INTO ${DynamicFieldsScreens.tableName} (${this.fields}, status, is_active) VALUES ($1, $2, $3, $4) RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [dynamicfieldsscreen[fieldList[0]], dynamicfieldsscreen[fieldList[1]], dynamicfieldsscreen.status, dynamicfieldsscreen.isActive]);
  ...
  static async update(id, dynamicfieldsscreen) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`UPDATE ${DynamicFieldsScreens.tableName} SET ${fieldList[0]} = $1, ${fieldList[1]} = $2, status = $3, is_active = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [dynamicfieldsscreen[fieldList[0]], dynamicfieldsscreen[fieldList[1]], dynamicfieldsscreen.status, dynamicfieldsscreen.isActive, id]);
