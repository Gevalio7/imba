const { pool } = require('../config/db');
  ...
  static async create(systemmaintenance) {
    try {
      ...
      values.push(systemmaintenance.status, systemmaintenance.isActive);
  ...
  static async update(id, systemmaintenance) {
    try {
      ...
      const values = fieldList.map(field => systemmaintenance[field]);
      values.push(systemmaintenance.status, systemmaintenance.isActive, id);
      const result = await pool.query(`UPDATE ${SystemMaintenance.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
