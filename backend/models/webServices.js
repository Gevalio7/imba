const { pool } = require('../config/db');
  ...
  static async create(webservice) {
    try {
      ...
      values.push(webservice.status, webservice.isActive);
  ...
  static async update(id, webservice) {
    try {
      ...
      const values = fieldList.map(field => webservice[field]);
      values.push(webservice.status, webservice.isActive, id);
      const result = await pool.query(`UPDATE ${WebServices.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
