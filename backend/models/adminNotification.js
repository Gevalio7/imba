const { pool } = require('../config/db');
  ...
  static async create(adminnotification) {
    try {
      ...
      values.push(adminnotification.status, adminnotification.isActive);
  ...
  static async update(id, adminnotification) {
    try {
      ...
      const values = fieldList.map(field => adminnotification[field]);
      values.push(adminnotification.status, adminnotification.isActive, id);
      const result = await pool.query(`UPDATE ${AdminNotification.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
