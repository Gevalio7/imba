const { pool } = require('../config/db');
  ...
  class EmailAddresses {
    ...
    static fields = 'email, description';
  ...
  static async create(emailaddresse) {
    try {
      ...
      values.push(emailaddresse.status, emailaddresse.isActive);
  ...
  static async update(id, emailaddresse) {
    try {
      ...
      const values = fieldList.map(field => emailaddresse[field]);
      values.push(emailaddresse.status, emailaddresse.isActive, id);
      const result = await pool.query(`UPDATE ${EmailAddresses.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
