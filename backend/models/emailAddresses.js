...
class EmailAddresses {
  ...
  static async getAll(options = {}) {
    ...
    try {
      ...
      return {
        emailAddresses: dataResult.rows,
        total,
      };
    } catch (error) {
  ...
  static async update(id, emailaddresse) {
    try {
      ...
      fieldList.forEach(field => {
        if (emailaddresse[field] !== undefined) {
          updates.push(`${toSnakeCase(field)} = $${paramIndex}`);
          values.push(emailaddresse[field]);
          paramIndex++;
        }
      });

      // Добавляем isActive если передан
      if (emailaddresse.isActive !== undefined) {
        updates.push(`is_active = $${paramIndex}`);
        values.push(emailaddresse.isActive);
        paramIndex++;
      }
