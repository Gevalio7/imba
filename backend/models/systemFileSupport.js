const { pool } = require('../config/db');
  ...
  static async create(systemfilesupport) {
    try {
      ...
      values.push(systemfilesupport.status || 1, systemfilesupport.isActive !== undefined ? systemfilesupport.isActive : true);
  ...
  static async update(id, systemfilesupport) {
    try {
      ...
      const values = fieldList.map(field => systemfilesupport[field]);
      values.push(systemfilesupport.status !== undefined ? systemfilesupport.status : undefined, systemfilesupport.isActive !== undefined ? systemfilesupport.isActive : undefined, id);
      const result = await pool.query(`UPDATE ${SystemFileSupport.tableName} SET ${setClause}${systemfilesupport.status !== undefined ? ', status = $${fieldList.length + 1}' : ''}${systemfilesupport.isActive !== undefined ? ', is_active = $${fieldList.length + ' + (systemfilesupport.status !== undefined ? 2 : 1) + '}' : ''}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + ' + (systemfilesupport.status !== undefined && systemfilesupport.isActive !== undefined ? 3 : systemfilesupport.status !== undefined || systemfilesupport.isActive !== undefined ? 2 : 1) + '} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
