...
class TestEntities {
  ...
  static async getAll(options = {}) {
    ...
    try {
      ...
      const dataQuery = `SELECT id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive" FROM ${TestEntities.tableName} ${whereClause} ${orderClause} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
  ...
  static async getById(id) {
    try {
      ...
      const result = await pool.query(
        `SELECT id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive" FROM ${TestEntities.tableName} WHERE id = $1`,
        [id]
      );
  ...
  static async create(testentity) {
    try {
      ...
      // Добавляем isActive
      values.push(testentity.isActive !== undefined ? testentity.isActive : true);
      ...
      const query = `INSERT INTO ${TestEntities.tableName} (${sqlFieldsInsert}, is_active) VALUES (${placeholders}, $${fieldList.length + 1}) RETURNING id, ${sqlFieldsSelect}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive"`;
  ...
  static async update(id, testentity) {
    try {
      ...
      fieldList.forEach(field => {
        if (testentity[field] !== undefined) {
          updates.push(`${toSnakeCase(field)} = $${paramIndex}`);
          values.push(testentity[field]);
          paramIndex++;
        }
      });

      // Добавляем isActive если передан
      if (testentity.isActive !== undefined) {
        updates.push(`is_active = $${paramIndex}`);
        values.push(testentity.isActive);
        paramIndex++;
      }
      ...
      const query = `UPDATE ${TestEntities.tableName} SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive"`;
      const result = await pool.query(query, values);
