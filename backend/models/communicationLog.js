const { pool } = require('../config/db');
  ...
  static async create(communicationlog) {
    try {
      ...
      values.push(communicationlog.status, communicationlog.isActive);
  ...
  static async update(id, communicationlog) {
    try {
      ...
      const values = fieldList.map(field => communicationlog[field]);
      values.push(communicationlog.status, communicationlog.isActive, id);
      const result = await pool.query(`UPDATE ${CommunicationLog.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
