const { pool } = require('../config/db');
  ...
  static async create(communicationlog) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`INSERT INTO ${CommunicationLog.tableName} (${this.fields}, status, is_active) VALUES ($1, $2, $3, $4) RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [communicationlog[fieldList[0]], communicationlog[fieldList[1]], communicationlog.status, communicationlog.isActive]);
  ...
  static async update(id, communicationlog) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`UPDATE ${CommunicationLog.tableName} SET ${fieldList[0]} = $1, ${fieldList[1]} = $2, status = $3, is_active = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [communicationlog[fieldList[0]], communicationlog[fieldList[1]], communicationlog.status, communicationlog.isActive, id]);
