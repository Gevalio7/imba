const { pool } = require('../config/db');
  ...
  static async create(appointmentnotification) {
    try {
      ...
      values.push(appointmentnotification.status, appointmentnotification.isActive);
  ...
  static async update(id, appointmentnotification) {
    try {
      ...
      const values = fieldList.map(field => appointmentnotification[field]);
      values.push(appointmentnotification.status, appointmentnotification.isActive, id);
      const result = await pool.query(`UPDATE ${AppointmentNotifications.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
