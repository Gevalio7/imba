const { pool } = require('../config/db');
  ...
  static async create(processesautomationsetting) {
    try {
      ...
      values.push(processesautomationsetting.status, processesautomationsetting.isActive);
      const result = await pool.query(`INSERT INTO ${ProcessesAutomationSettings.tableName} (${this.fields}, status, is_active) VALUES (${placeholders}, $${fieldList.length + 1}, $${fieldList.length + 2}) RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
  ...
  static async update(id, processesautomationsetting) {
    try {
      ...
      const values = fieldList.map(field => processesautomationsetting[field]);
      values.push(processesautomationsetting.status, processesautomationsetting.isActive, id);
      const result = await pool.query(`UPDATE ${ProcessesAutomationSettings.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
