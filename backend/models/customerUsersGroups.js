const { pool } = require('../config/db');
  ...
  static async create(customerusersgroup) {
    try {
      ...
      values.push(customerusersgroup.status, customerusersgroup.isActive);
  ...
  static async update(id, customerusersgroup) {
    try {
      ...
      const values = fieldList.map(field => customerusersgroup[field]);
      values.push(customerusersgroup.status, customerusersgroup.isActive, id);
      const result = await pool.query(`UPDATE ${CustomerUsersGroups.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
