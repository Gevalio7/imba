const { pool } = require('../config/db');
  ...
  static async create(templatequeue) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`INSERT INTO ${TemplateQueues.tableName} (${this.fields}, status, is_active) VALUES ($1, $2, $3, $4) RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [templatequeue[fieldList[0]], templatequeue[fieldList[1]], templatequeue.status, templatequeue.isActive]);
  ...
  static async update(id, templatequeue) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`UPDATE ${TemplateQueues.tableName} SET ${fieldList[0]} = $1, ${fieldList[1]} = $2, status = $3, is_active = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [templatequeue[fieldList[0]], templatequeue[fieldList[1]], templatequeue.status, templatequeue.isActive, id]);
