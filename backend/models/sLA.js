const { pool } = require('../config/db');
  ...
  static async create(sla) {
    try {
      ...
      values.push(sla.status, sla.isActive);
  ...
  static async update(id, sla) {
    try {
      ...
      const values = fieldList.map(field => sla[field]);
      values.push(sla.status, sla.isActive, id);
      const result = await pool.query(`UPDATE ${SLA.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
