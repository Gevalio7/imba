const { pool } = require('../config/db');
  ...
  static async create(smimecertificate) {
    try {
      ...
      values.push(smimecertificate.status, smimecertificate.isActive);
  ...
  static async update(id, smimecertificate) {
    try {
      ...
      const values = fieldList.map(field => smimecertificate[field]);
      values.push(smimecertificate.status, smimecertificate.isActive, id);
      const result = await pool.query(`UPDATE ${SmimeCertificates.tableName} SET ${setClause}, status = $${fieldList.length + 1}, is_active = $${fieldList.length + 2}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + 3} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
