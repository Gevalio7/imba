const { pool } = require('../config/db');
  ...
  static async create(customerusersservice) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`INSERT INTO ${CustomerUsersServices.tableName} (${this.fields}, status, is_active) VALUES ($1, $2, $3, $4) RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [customerusersservice[fieldList[0]], customerusersservice[fieldList[1]], customerusersservice.status, customerusersservice.isActive]);
  ...
  static async update(id, customerusersservice) {
    try {
      const fieldList = this.fields.split(', ');
      const result = await pool.query(`UPDATE ${CustomerUsersServices.tableName} SET ${fieldList[0]} = $1, ${fieldList[1]} = $2, status = $3, is_active = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, [customerusersservice[fieldList[0]], customerusersservice[fieldList[1]], customerusersservice.status, customerusersservice.isActive, id]);
