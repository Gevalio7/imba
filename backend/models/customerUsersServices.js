const { pool } = require('../config/db');
  ...
  static async create(customerusersservice) {
    try {
      ...
      values.push(customerusersservice.status || 1, customerusersservice.isActive !== undefined ? customerusersservice.isActive : true);
  ...
  static async update(id, customerusersservice) {
    try {
      ...
      const values = fieldList.map(field => customerusersservice[field]);
      values.push(customerusersservice.status !== undefined ? customerusersservice.status : undefined, customerusersservice.isActive !== undefined ? customerusersservice.isActive : undefined, id);
      const result = await pool.query(`UPDATE ${CustomerUsersServices.tableName} SET ${setClause}${customerusersservice.status !== undefined ? ', status = $${fieldList.length + 1}' : ''}${customerusersservice.isActive !== undefined ? ', is_active = $${fieldList.length + ' + (customerusersservice.status !== undefined ? 2 : 1) + '}' : ''}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fieldList.length + ' + (customerusersservice.status !== undefined && customerusersservice.isActive !== undefined ? 3 : customerusersservice.status !== undefined || customerusersservice.isActive !== undefined ? 2 : 1) + '} RETURNING id, ${this.fields}, created_at as "createdAt", updated_at as "updatedAt", status, is_active as "isActive"`, values);
