...
class PgpKeys {
  ...
  static async getAll(options = {}) {
    ...
    try {
      ...
      const dataQuery = `SELECT id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive" FROM ${PgpKeys.tableName} ${whereClause} ${orderClause} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
      params.push(itemsPerPage, offset);
      const dataResult = await pool.query(dataQuery, params);

      return {
        pgpKeys: dataResult.rows,
        total,
      };
    } catch (error) {
  ...
  static async getById(id) {
    try {
      ...
      const result = await pool.query(
        `SELECT id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive" FROM ${PgpKeys.tableName} WHERE id = $1`,
        [id]
      );
  ...
  static async create(pgpkey) {
    try {
      ...
      // Добавляем isActive
      values.push(pgpkey.isActive !== undefined ? pgpkey.isActive : true);
      ...
      const query = `INSERT INTO ${PgpKeys.tableName} (${sqlFieldsInsert}, is_active) VALUES (${placeholders}, $${fieldList.length + 1}) RETURNING id, ${sqlFieldsSelect}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive"`;
  ...
  static async update(id, pgpkey) {
    try {
      ...
      fieldList.forEach(field => {
        if (pgpkey[field] !== undefined) {
          updates.push(`${toSnakeCase(field)} = $${paramIndex}`);
          values.push(pgpkey[field]);
          paramIndex++;
        }
      });

      // Добавляем isActive если передан
      if (pgpkey.isActive !== undefined) {
        updates.push(`is_active = $${paramIndex}`);
        values.push(pgpkey.isActive);
        paramIndex++;
      }
      ...
      const query = `UPDATE ${PgpKeys.tableName} SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING id, ${sqlFields}, created_at as "createdAt", updated_at as "updatedAt", is_active as "isActive"`;
      const result = await pool.query(query, values);
