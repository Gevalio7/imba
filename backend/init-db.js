...
const masterPool = new pg_1.Pool({
    host: 'localhost',
    port: 5432,
    user: 'postgres',
    password: 'postgres',
...
async function createTableFromConfig(pool, tableName, config) {
...
async function insertInitialData(pool, tableName, config) {
...
async function initializeDatabase() {
    try {
        ...
        const configPath = path_1.default.join(__dirname, 'entities-config.json');
        const entitiesConfig = JSON.parse(fs_1.default.readFileSync(configPath, 'utf-8'));
