module.exports = {
  up: async (pool) => {
    ...
    const check = await pool.query(
      `SELECT id FROM system_configuration WHERE name = 'reference_data_cache_ttl_seconds'`
    )

    if (check.rows.length === 0) {
      await pool.query(`
        INSERT INTO system_configuration (name, value, description, config_type, is_editable, is_active, created_at, updated_at)
        VALUES ('reference_data_cache_ttl_seconds', '300', 'Время жизни кеша referenceData (сек). 0 = кеш отключён', 'system', false, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `)
    ...
    const checkEnabled = await pool.query(
      `SELECT id FROM system_configuration WHERE name = 'reference_data_cache_enabled'`
    )

    if (checkEnabled.rows.length === 0) {
      await pool.query(`
        INSERT INTO system_configuration (name, value, description, config_type, is_editable, is_active, created_at, updated_at)
        VALUES ('reference_data_cache_enabled', 'true', 'Включить кеширование referenceData', 'system', false, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `)
