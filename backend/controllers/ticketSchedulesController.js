...
exports.getById = async (req, res) => {
  try {
    const { id } = req.params;
    const schedule = await TicketSchedules.getById(parseInt(id, 10));
    if (!schedule) {
...
exports.getByTicketId = async (req, res) => {
  try {
    ...
    const schedule = await TicketSchedules.getByTicketId(parseInt(ticketId, 10));
    res.json({ schedule });
  } catch (error) {
...
exports.update = async (req, res) => {
  try {
    ...
    const existing = await TicketSchedules.getById(parseInt(id, 10));
    if (!existing) {
      return res.status(404).json({ message: 'Расписание не найдено' });
    }
    
    const schedule = await TicketSchedules.update(parseInt(id, 10), scheduleData);
    res.json({ schedule });
  } catch (error) {
...
exports.delete = async (req, res) => {
  try {
    ...
    const deleted = await TicketSchedules.delete(parseInt(id, 10));
...
exports.deleteByTicketId = async (req, res) => {
  try {
    ...
    const deleted = await TicketSchedules.deleteByTicketId(parseInt(ticketId, 10));
    res.json({ success: deleted });
  } catch (error) {
