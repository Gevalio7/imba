...
const createTicket = asyncHandler(async (req, res) => {
  ...
  if (ownerId && typeof ownerId === 'string' && ownerId.includes('@')) {
    ...
    if (shouldCreateUser) {
      const existingUser = await CustomerUsers.getByEmail(ownerId)
    ...
    if (shouldCreateUser) {
      const existingUser = await CustomerUsers.getByEmail(ownerId.email)
  ...
  if (req.body.responseDeadline)
    data.responseDeadline = req.body.responseDeadline
  if (req.body.resolutionDeadline)
    data.resolutionDeadline = req.body.resolutionDeadline
  if (req.body.firstResponseAt)
    data.firstResponseAt = req.body.firstResponseAt
  if (req.body.slaViolated !== undefined)
    data.slaViolated = req.body.slaViolated
  if (req.body.pendingStartAt)
    data.pendingStartAt = req.body.pendingStartAt
  ...
  if (req.body.escalationCount !== undefined)
    data.escalationCount = req.body.escalationCount
  if (req.body.isEscalated !== undefined)
    data.isEscalated = req.body.isEscalated
...
const updateTicket = asyncHandler(async (req, res) => {
  ...
  const data = {}
  if (req.body.ticketNumber !== undefined)
    data.ticketNumber = req.body.ticketNumber
  ...
  // SLA поля
  if (req.body.responseDeadline !== undefined)
    data.responseDeadline = req.body.responseDeadline
  if (req.body.resolutionDeadline !== undefined)
    data.resolutionDeadline = req.body.resolutionDeadline
  if (req.body.firstResponseAt !== undefined)
    data.firstResponseAt = req.body.firstResponseAt
  if (req.body.slaViolated !== undefined)
    data.slaViolated = req.body.slaViolated
  if (req.body.pendingStartAt !== undefined)
    data.pendingStartAt = req.body.pendingStartAt
  ...
  for (const field of fieldsToTrack) {
    if (data[field] !== undefined && currentTicket[field] !== data[field]) {
      if (field === 'typeId')
        neededLookups.add('types')
      else if (field === 'categoryId')
        neededLookups.add('typeCategories')
      else if (field === 'priorityId')
        neededLookups.add('priorities')
      else if (field === 'queueId')
        neededLookups.add('queues')
      else if (field === 'stateId')
        neededLookups.add('states')
      else if (field === 'ownerId')
        neededLookups.add('agents')
      else if (field === 'companyId')
        neededLookups.add('customers')
      else if (field === 'serviceId')
        neededLookups.add('services')
      else if (field === 'slaId')
        neededLookups.add('sla')
      else if (field === 'executorGroupIds')
        neededLookups.add('agentsGroups')
    }
  }
  ...
  lookupOrder.forEach((key, index) => {
    ...
    if (key === 'types')
      lookupData.typesList = result.types || []
    else if (key === 'typeCategories')
      lookupData.typeCategoriesList = result.typeCategories || []
    else if (key === 'priorities')
      lookupData.prioritiesList = result.priorities || []
    else if (key === 'queues')
      lookupData.queuesList = result.queues || []
    else if (key === 'states')
      lookupData.statesList = result.states || []
    else if (key === 'agents')
      lookupData.agentsList = result.agents || []
    else if (key === 'customers')
      lookupData.customersList = result.customers || []
    else if (key === 'services')
      lookupData.servicesList = result.services || []
    else if (key === 'sla')
      lookupData.slaList = result.sla || []
    else if (key === 'agentsGroups')
      lookupData.agentsGroupsList = result.agentsGroups || []
  })
