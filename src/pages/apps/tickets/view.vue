...
const ownerDisplay = computed(() => {
  ...
  try {
    if (typeof owner === 'object') {
      ...
      if ((owner as any).title) return (owner as any).title
      ...
      if (name) return `${name}${email}`.trim()
      if (login) return `${login}${email}`.trim()
    }
  ...
  <div>
    <div class="d-flex flex-wrap justify-start justify-sm-space-between gap-y-4 gap-x-6 mb-6">
      <div class="d-flex flex-column justify-center">
        <h4 class="text-h4 mb-1">Просмотр обращения</h4>
        <div class="text-body-1">#{{ ticket.ticketNumber }}</div>
      </div>
      <div class="d-flex gap-4 align-center flex-wrap">
        <VBtn variant="tonal" color="secondary" @click="goBack">Назад</VBtn>
      </div>
    </div>
      ...
      <VCol cols="12" md="8">
        <VCard class="mb-6">
          <VCardTitle class="d-flex align-center px-6 py-4">
            <h5 class="text-h5">Основная информация</h5>
          </VCardTitle>
          <VCardText>
            <div class="mb-4">
              <h6 class="text-subtitle-1">Заголовок</h6>
              <div class="text-body-1">{{ ticket.title || '-' }}</div>
            </div>

            <div class="mb-4">
              <h6 class="text-subtitle-1">Описание</h6>
              <div v-if="description" class="prose" v-html="description"></div>
              <div v-else class="text-body-2 text-medium-emphasis">Нет описания</div>
            </div>

            <div class="mb-4">
              <h6 class="text-subtitle-1">Вложения</h6>
              <div v-if="ticket.id">
                <!-- Простое отображение списка вложений будет загружаться отдельно через API при необходимости -->
                <div class="text-body-2 text-medium-emphasis">Вложения можно просмотреть на странице редактирования (если есть права)</div>
              </div>
