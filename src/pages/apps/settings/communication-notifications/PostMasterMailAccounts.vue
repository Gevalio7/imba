...
// Типы данных для почтовой учетной записи PostMaster
...
// Данные почтовых учетных записей PostMaster (демо данные)
const postMasterMailAccounts = ref<PostMasterMailAccount[]>([
  {
    id: 1,
    name: 'support@company.com',
    message: 'Учетная запись для получения обращений поддержки',
  ...
  },
  {
    ...
    message: 'Учетная запись для получения запросов продаж',
  ...
  },
  {
    ...
    message: 'Общая учетная запись для информационных запросов',
  ...
  },
  {
    ...
    message: 'Учетная запись для финансовых вопросов',
  ...
  {
    id: 5,
    name: 'archive@company.com',
    message: 'Архивная учетная запись, больше не используется',
  ...
  },
  {
    ...
    message: 'Учетная запись для сбора отзывов клиентов',
...
const confirmBulkDelete = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = postMasterMailAccounts.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      postMasterMailAccounts.value.splice(index, 1)
    }
  })
  selectedItems.value = []
  showToast(`Удалено ${count} почтовых учетных записей`)
...
const confirmBulkStatusChange = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = postMasterMailAccounts.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      postMasterMailAccounts.value[index].status = bulkStatusValue.value
      postMasterMailAccounts.value[index].isActive = bulkStatusValue.value === 1
    }
  })
  selectedItems.value = []
  showToast(`Статус изменен для ${count} почтовых учетных записей`)
...
const save = () => {
  ...
  if (editedIndex.value > -1) {
    ...
    Object.assign(postMasterMailAccounts.value[editedIndex.value], editedItem.value)
    showToast('Почтовая учетная запись успешно сохранена')
  } else {
    // Добавление нового
    const newId = Math.max(...postMasterMailAccounts.value.map(t => t.id)) + 1
    const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
    editedItem.value.id = newId
    editedItem.value.createdAt = now
    editedItem.value.updatedAt = now
    postMasterMailAccounts.value.push({ ...editedItem.value })
    showToast('Почтовая учетная запись успешно добавлена')
  }
  close()
}

const deleteItemConfirm = () => {
  postMasterMailAccounts.value.splice(editedIndex.value, 1)
  showToast('Почтовая учетная запись успешно удалена')
  closeDelete()
}

// Переключение статуса
const toggleStatus = (item: PostMasterMailAccount, newValue: number) => {
  console.log('🔄 toggleStatus вызван')
  console.log('📝 Элемент:', item)
  console.log('🔢 Новое значение статуса:', newValue)
  
  const index = postMasterMailAccounts.value.findIndex((t: PostMasterMailAccount) => t.id === item.id)
  console.log('🔍 Найденный индекс:', index)
  
  if (index !== -1) {
    console.log('✅ Элемент найден, обновляем статус')
    postMasterMailAccounts.value[index].status = newValue
    postMasterMailAccounts.value[index].isActive = newValue === 1
    console.log('✅ Обновленный элемент:', postMasterMailAccounts.value[index])
    showToast('Статус почтовой учетной записи изменен')
  } else {
    console.error('❌ Элемент не найден в массиве postMasterMailAccounts')
  }
}
...
// Добавление новой почтовой учетной записи
...
<template>
  <div>
    <VCard title="Учетные записи почты для PostMaster">

      <div class="d-flex flex-wrap gap-4 pa-6">
        <div class="d-flex align-center">
          <!-- Поиск -->
          <AppTextField
            placeholder="Поиск почтовых учетных записей"
        ...
        <div class="d-flex gap-4 flex-wrap align-center">
          ...
          >
            Добавить почтовую учетную запись
          </VBtn>
        </div>
      </div>
      ...
      >
        <VCard title="Подтверждение удаления">
          <VCardText>
            Вы уверены, что хотите удалить выбранные почтовые учетные записи? Это действие нельзя отменить.
          </VCardText>
          <VCardText>
            <div class="d-flex justify-end gap-4">
              <VBtn
    ...
    <VDialog
      ...
      max-width="600px"
    >
      <VCard :title="editedIndex > -1 ? 'Редактировать почтовую учетную запись' : 'Добавить почтовую учетную запись'">
        <VCardText>
          <VRow>
            <!-- Название -->
            <VCol
            ...
            <VCol cols="12">
              <AppTextarea
                ...
                label="Описание почтовой учетной записи"
                rows="4"
                placeholder="Введите описание учетной записи..."
              />
            </VCol>
    ...
    <VDialog
      ...
      max-width="500px"
    >
      <VCard title="Вы уверены, что хотите удалить эту почтовую учетную запись?">
        <VCardText>
