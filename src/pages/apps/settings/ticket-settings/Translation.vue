...
const defaultItem = ref<Translation>({
  id: -1,
  name: '',
  message: '',
  createdAt: '',
...
const editedItem = ref<Translation>({ ...defaultItem.value })
...
const toggleStatus = async (item: Translation, newValue: boolean | null) => {
...
<template>
  <div>
    <VCard title="Переводы">
      ...
      >
        <VCard title="Подтверждение удаления">
          <VCardText>
            Вы уверены, что хотите удалить выбранные переводы? Это действие нельзя отменить.
          </VCardText>
          <VCardText>
            <div class="d-flex justify-end gap-4">
              <VBtn
    ...
    <VDialog
      ...
      max-width="500px"
    >
      <VCard title="Вы уверены, что хотите удалить этот перевод?">
        <VCardText>
