...
// Типы данных для очереди шаблона
...
// Данные очередей шаблонов (демо данные)
const templateQueues = ref<TemplateQueue[]>([
  {
    id: 1,
...
const confirmBulkDelete = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = templateQueues.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      templateQueues.value.splice(index, 1)
    }
  })
  selectedItems.value = []
  showToast(`Удалено ${count} очередей шаблонов`)
...
const confirmBulkStatusChange = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = templateQueues.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      templateQueues.value[index].status = bulkStatusValue.value
      templateQueues.value[index].isActive = bulkStatusValue.value === 1
    }
  })
  selectedItems.value = []
  showToast(`Статус изменен для ${count} очередей шаблонов`)
...
const defaultItem = ref<TemplateQueue>({
  id: -1,
  name: '',
  message: '',
  createdAt: '',
...
const editedItem = ref<TemplateQueue>({ ...defaultItem.value })
...
// Методы
const editItem = (item: TemplateQueue) => {
  editedIndex.value = templateQueues.value.indexOf(item)
  editedItem.value = { ...item }
  editDialog.value = true
}

const deleteItem = (item: TemplateQueue) => {
  editedIndex.value = templateQueues.value.indexOf(item)
  editedItem.value = { ...item }
...
const save = () => {
  ...
  if (editedIndex.value > -1) {
    ...
    Object.assign(templateQueues.value[editedIndex.value], editedItem.value)
    showToast('Очередь шаблона успешно сохранена')
  } else {
    // Добавление нового
    const newId = Math.max(...templateQueues.value.map(t => t.id)) + 1
    const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
    editedItem.value.id = newId
    editedItem.value.createdAt = now
    editedItem.value.updatedAt = now
    templateQueues.value.push({ ...editedItem.value })
    showToast('Очередь шаблона успешно добавлена')
  }
  close()
}

const deleteItemConfirm = () => {
  templateQueues.value.splice(editedIndex.value, 1)
  showToast('Очередь шаблона успешно удалена')
  closeDelete()
}

// Переключение статуса
const toggleStatus = (item: TemplateQueue, newValue: number) => {
  console.log('🔄 toggleStatus вызван')
  console.log('📝 Элемент:', item)
  console.log('🔢 Новое значение статуса:', newValue)
  
  const index = templateQueues.value.findIndex((t: TemplateQueue) => t.id === item.id)
  console.log('🔍 Найденный индекс:', index)
  
  if (index !== -1) {
    console.log('✅ Элемент найден, обновляем статус')
    templateQueues.value[index].status = newValue
    templateQueues.value[index].isActive = newValue === 1
    console.log('✅ Обновленный элемент:', templateQueues.value[index])
    showToast('Статус очереди шаблона изменен')
  } else {
    console.error('❌ Элемент не найден в массиве templateQueues')
  }
}
...
// Добавление новой очереди шаблона
const addNewTemplateQueue = () => {
  editedItem.value = { ...defaultItem.value }
  editedIndex.value = -1
  editDialog.value = true
}
...
<template>
  <div>
    <VCard title="Шаблоны ↔️ Очереди">

      <div class="d-flex flex-wrap gap-4 pa-6">
        <div class="d-flex align-center">
          <!-- Поиск -->
          <AppTextField
            placeholder="Поиск очередей шаблонов"
          ...
          <VRow>
            ...
            <VCol cols="12">
              <AppTextarea
                ...
                label="Сообщение очереди шаблона"
                rows="4"
                placeholder="Введите текст очереди шаблона..."
              />
            </VCol>
    ...
    <VDialog
      ...
      max-width="500px"
    >
      <VCard title="Вы уверены, что хотите удалить эту очередь шаблона?">
        <VCardText>
