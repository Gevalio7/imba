...
// Типы данных для группы клиентов
...
// Данные групп клиентов (демо данные)
const customerGroups = ref<CustomerGroup[]>([
  {
    id: 1,
    name: 'vip-clients',
    message: 'Группа VIP клиентов с приоритетной поддержкой',
  ...
  },
  {
    ...
    message: 'Группа обычных клиентов',
  ...
  },
  {
    ...
    message: 'Группа корпоративных клиентов',
  ...
  },
  {
    id: 4,
    name: 'trial-clients',
    message: 'Группа клиентов на пробном периоде',
  ...
  },
  {
    ...
    message: 'Группа неактивных клиентов',
  ...
  },
  {
    ...
    message: 'Группа клиентов для сбора отзывов',
...
const confirmBulkDelete = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = customerGroups.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
...
const confirmBulkStatusChange = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = customerGroups.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      customerGroups.value[index].status = bulkStatusValue.value
      customerGroups.value[index].isActive = bulkStatusValue.value === 1
    }
  })
  selectedItems.value = []
...
const defaultItem = ref<CustomerGroup>({
  id: -1,
  name: '',
  message: '',
  createdAt: '',
...
const editedItem = ref<CustomerGroup>({ ...defaultItem.value })
...
// Методы
const editItem = (item: CustomerGroup) => {
  editedIndex.value = customerGroups.value.indexOf(item)
  editedItem.value = { ...item }
  editDialog.value = true
}

const deleteItem = (item: CustomerGroup) => {
  editedIndex.value = customerGroups.value.indexOf(item)
  editedItem.value = { ...item }
...
const save = () => {
  ...
  if (editedIndex.value > -1) {
    ...
    Object.assign(customerGroups.value[editedIndex.value], editedItem.value)
    showToast('Группа клиентов успешно сохранена')
  } else {
    // Добавление нового
    const newId = Math.max(...customerGroups.value.map(t => t.id)) + 1
    const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
    editedItem.value.id = newId
    editedItem.value.createdAt = now
    editedItem.value.updatedAt = now
    customerGroups.value.push({ ...editedItem.value })
...
const deleteItemConfirm = () => {
  customerGroups.value.splice(editedIndex.value, 1)
  showToast('Группа клиентов успешно удалена')
  closeDelete()
}

// Переключение статуса
const toggleStatus = (item: CustomerGroup, newValue: number) => {
  console.log('🔄 toggleStatus вызван')
  console.log('📝 Элемент:', item)
  console.log('🔢 Новое значение статуса:', newValue)
  
  const index = customerGroups.value.findIndex((t: CustomerGroup) => t.id === item.id)
  console.log('🔍 Найденный индекс:', index)
  
  if (index !== -1) {
    console.log('✅ Элемент найден, обновляем статус')
    customerGroups.value[index].status = newValue
    customerGroups.value[index].isActive = newValue === 1
    console.log('✅ Обновленный элемент:', customerGroups.value[index])
    showToast('Статус группы клиентов изменен')
  } else {
    console.error('❌ Элемент не найден в массиве customerGroups')
  }
}
  ...
  <div>
    <VCard title="Группы клиентов">
      ...
      <div class="d-flex justify-center mt-4 pb-4">
        <VPagination
          v-model="currentPage"
          :length="Math.ceil(filteredCustomerGroups.length / itemsPerPage) || 1"
    ...
    <VDialog
      ...
      max-width="600px"
    >
      <VCard :title="editedIndex > -1 ? 'Редактировать группу клиентов' : 'Добавить группу клиентов'">
        <VCardText>
          <VRow>
            <!-- Название -->
            <VCol
            ...
            <VCol cols="12">
              <AppTextarea
                ...
                label="Описание группы клиентов"
                rows="4"
                placeholder="Введите описание группы клиентов..."
              />
            </VCol>
