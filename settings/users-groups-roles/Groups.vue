...
// Данные групп (демо данные)
const groups = ref<Group[]>([
  {
    id: 1,
    name: 'admins',
    message: 'Группа администраторов с полными правами',
  ...
  },
  {
    id: 2,
    name: 'managers',
    message: 'Группа менеджеров для управления проектами',
  ...
  },
  {
    id: 3,
    name: 'users',
    message: 'Базовая группа пользователей',
  ...
  },
  {
    id: 4,
    name: 'support',
    message: 'Группа поддержки клиентов',
  ...
  },
  {
    id: 5,
    name: 'guests',
    message: 'Группа гостей с ограниченными правами',
  ...
  },
  {
    id: 6,
    name: 'analysts',
    message: 'Группа аналитиков для отчетов',
...
const confirmBulkDelete = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = groups.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      groups.value.splice(index, 1)
    }
  })
  selectedItems.value = []
  showToast(`Удалено ${count} групп`)
...
const confirmBulkStatusChange = () => {
  ...
  selectedItems.value.forEach(item => {
    const index = groups.value.findIndex(t => t.id === item.id)
    if (index !== -1) {
      groups.value[index].status = bulkStatusValue.value
      groups.value[index].isActive = bulkStatusValue.value === 1
    }
  })
  selectedItems.value = []
  showToast(`Статус изменен для ${count} групп`)
...
const editedItem = ref<Group>({ ...defaultItem.value })
...
// Методы
const editItem = (item: Group) => {
  editedIndex.value = groups.value.indexOf(item)
  editedItem.value = { ...item }
  editDialog.value = true
}

const deleteItem = (item: Group) => {
  editedIndex.value = groups.value.indexOf(item)
  editedItem.value = { ...item }
...
const save = () => {
  ...
  if (editedIndex.value > -1) {
    ...
    // Добавление нового
    const newId = Math.max(...groups.value.map(t => t.id)) + 1
...
const toggleStatus = (item: Group, newValue: number) => {
  console.log('🔄 toggleStatus вызван')
  console.log('📝 Элемент:', item)
  console.log('🔢 Новое значение статуса:', newValue)
  
  const index = groups.value.findIndex((t: Group) => t.id === item.id)
  console.log('🔍 Найденный индекс:', index)
  
  if (index !== -1) {
    console.log('✅ Элемент найден, обновляем статус')
    groups.value[index].status = newValue
    groups.value[index].isActive = newValue === 1
    console.log('✅ Обновленный элемент:', groups.value[index])
    showToast('Статус группы изменен')
  } else {
    console.error('❌ Элемент не найден в массиве groups')
  }
}
